...
function isAlongRouteInOrder(pickup: Location, dropoff: Location, roadPoints: [number, number][], radiusKm: number = 2.0): boolean {
  ...
  console.log(`   📏 Pickup Dist: ${pickupMatch.minDistance.toFixed(2)}km | Dropoff Dist: ${dropoffMatch.minDistance.toFixed(2)}km`);

  // FIX: Using explicit !(distance <= radius) ensures that if distance is Infinity or NaN, it is accurately REJECTED.
  if (!(pickupMatch.minDistance <= radiusKm)) {
    console.log(`   ❌ REJECTED: Pickup is too far (${pickupMatch.minDistance.toFixed(2)}km)`);
    return false;
  }
  if (!(dropoffMatch.minDistance <= radiusKm)) {
    console.log(`   ❌ REJECTED: Dropoff is too far (${dropoffMatch.minDistance.toFixed(2)}km)`);
    return false;
  }
...
function isTimeWithinWindow(time1: string, time2: string, windowMinutes: number = 1440): boolean {
  ...
  if (!isMatch) console.log(`   ❌ REJECTED: Time mismatch. Difference is ${diffMinutes.toFixed(0)} mins`);
  return isMatch;
}
...
export async function findMatches(): Promise<MatchResult[]> {
  ...
  console.log(`\n🔍 MATCH ENGINE RUNNING: ${pendingRequests.length} requests, ${providerRoutes.length} routes.`);
  ...
  for (const request of pendingRequests) {
    ...
    console.log(`\n📦 Checking Request #${request.request_id} ...`);
...
export async function autoMatchAndCreateDeliveries(): Promise<any[]> {
  ...
  for (const match of matches) {
    ...
    console.log(`   🎉 Match fully processed! Delivery ID: ${delivery.delivery_id}`);
